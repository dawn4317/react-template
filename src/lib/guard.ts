...
interface SecanEvent extends Event {
	detail?: {
		args: any;
		invalidScript: any;
	};
...
export default function guard(): void {
	...
	window.addEventListener('eval', (e: SecanEvent): void => {
		report(new Error(`Eval called, args is: ${JSON.stringify(e.detail!.args)}`));
	});
	window.addEventListener('alert', (e: SecanEvent) => {
		report(new Error(`alert called, args is: ${JSON.stringify(e.detail!.args)}`));
	});
	window.addEventListener('console', (e: SecanEvent) => {
